...
router.get('/download', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const data = await fileService.download(req.body.userId, req.body.fileName);
        res.status(200).json(data);
    } catch (error) {
        next(error);
    }
...
router.put('/update', upload.single('file'), async (req: Request, res: Response, next: NextFunction) => {
    try {
        const data = await fileService.update(req.body.userId, req.body.fileName, req.file);
        res.status(200).json(data);
    } catch (error) {
        next(error);
    }
